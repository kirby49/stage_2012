...
void MyWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    {
        m_pointList.erase(m_pointList.remove(indexPoint));
    }
    else
    {
        if(checkLines(point))
        ...
        {
             m_pointList.insert(indexPoint,point);
        }
    }
...
bool MyWidget::checkLines(QPoint checkpoint)
{
    if(!m_pointList.isEmpty())
    {
        ...
        {
            int dif,p,m;
            if(m_pointList[i].x()-m_pointList[i+1].x() !=0)
            {
                m = (m_pointList[i].y()-m_pointList[i+1].y())/(m_pointList[i].x()-m_pointList[i+1].x());
                p = m_pointList[i].y()-m*m_pointList[i].x();
                dif = m*checkpoint.x()-checkpoint.y()+p/sqrt(double(1+m*m));
            }else{
                dif = checkpoint.x()-m_pointList[i].y();
            }
            if(dif < 10 && dif >-10)
            {
                indexPoint = i+1;
